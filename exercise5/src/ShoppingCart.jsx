...
const ShoppingCart = () => {
  ...
  const handleAddCart = () => {
    if(cartInput.name.trim() === "" || isNaN(Number(cartInput.price))) {
        return;
    }
  ...
  const handleDecreaseQty= (id) => {
    setListCart(
        listCart.map((item) => item.id === id && item.qty > 1 ? { ...item, qty:item.qty-1}:item)
    )
  }
  ...
  const TotalPrice=listCart.reduce((total, item)=>total+Number(item.price)*item.qty, 0);
  return (
    <div>
      ...
      <ul>
        ...
        listCart.map((item) => (
          ...
          <li key={item.id}>
            ...
            <p>
              Quantity: <button onClick={()=> handleDecreaseQty(item.id)}>-</button>
              {item.qty}
